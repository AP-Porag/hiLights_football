...
interface Summary {
    ...
    topClub: { name: string; organization: string | null; count: number; avatar: string | null } | null;
}
...
interface PageProps {
    ...
    mostActiveClubs: ActiveClub[];
    allClubs: Paginator<ClubListItem>;
    clubSearch: string;
...
export default function ClubIndex() {
    ...
    const summary = page.summary ?? { avgRating: 0, totalRatings: 0, totalPlayers: 0, ratingsThisMonth: 0, topClub: null };
    const mostRatedPlayers = page.mostRatedPlayers ?? [];
    const mostActiveClubs = page.mostActiveClubs ?? [];
    const allClubs = page.allClubs ?? { data: [], current_page: 1, last_page: 1, from: 0, to: 0, total: 0 };
    const clubSearchInitial = page.clubSearch ?? '';
    ...
    const [clubSearch, setClubSearch] = useState(clubSearchInitial);
        ...
        const t = setTimeout(() => {
            router.get(
                route('clubs.index'),
                {
                    ...
                    club_search: clubSearch || undefined,
                },
    ...
    const goToClubPage = (page: number) => {
        router.get(
            route('clubs.index'),
            {
                club_page: page,
    ...
    const handleDelete = () => {
        ...
        setDeleting(true);
        router.delete(route('clubs.destroy', deleteRating.id), {
            preserveScroll: true,
            onFinish: () => {
                setDeleting(false);
    ...
    const clubPageWindow: number[] = [];
    {
        const start = Math.max(1, allClubs.current_page - 1);
        const end = Math.min(allClubs.last_page, allClubs.current_page + 1);
        for (let i = start; i <= end; i++) clubPageWindow.push(i);
    }
                        ...
                        <div className="overflow-x-auto">
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    {allClubs.data.map((club) => (
                                        <TableRow key={club.id} className="border-[#2A2A2A] hover:bg-[#1A1A1A]">
                                            ...
                                            <TableCell className="py-4 text-sm text-[#E2E8F0]">{club.email}</TableCell>
                                            <TableCell className="py-4 text-sm text-[#E2E8F0]">{club.total_ratings}</TableCell>
                                            <TableCell className="py-4 text-sm text-[#E2E8F0]">
                                                {club.avg_rating > 0 ? (
                                                    <div className="flex items-center gap-2">
                                                        <span>{club.avg_rating.toFixed(1)}</span>
                        ...
                        <div className="flex flex-col items-start gap-3 border-t border-[#2A2A2A] p-6 sm:flex-row sm:items-center sm:justify-between">
                            <div className="text-xs text-[#94A3B8]">
                                ...
                                <span className="font-mono font-semibold text-white">
                                    {allClubs.from ?? 0}-{allClubs.to ?? 0}
                                </span>{' '}
                                of{' '}
                                <span className="font-mono font-semibold text-white">
                                    {allClubs.total.toLocaleString()}
                                </span>{' '}
                                clubs
                            </div>
                            <div className="flex items-center gap-2">
                                <Button
                                    variant="outline"
                                    size="sm"
                                    disabled={allClubs.current_page <= 1}
                                    onClick={() => goToClubPage(allClubs.current_page - 1)}
                                ...
                                <div className="flex items-center gap-1">
                                    {clubPageWindow.map((p) =>
                                        p === allClubs.current_page ? (
                                            <Button
                                                key={p}
                                                size="sm"
                                ...
                                <Button
                                    variant="outline"
                                    size="sm"
                                    disabled={allClubs.current_page >= allClubs.last_page}
                                    onClick={() => goToClubPage(allClubs.current_page + 1)}
                                    className="border-[#2A2A2A] bg-[#1A1A1A] text-white hover:bg-[#2A2A2A] disabled:opacity-50"
                                >
                                    Next
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <Card className="border-[#2A2A2A] bg-[#0F0F0F]">
                        <CardContent className="p-6">
                            <div className="flex items-start justify-between">
                                <div className="flex-1">
                                    <div className="text-xs font-medium uppercase tracking-wider text-[#94A3B8]">Top Club</div>
                                    {summary.topClub ? (
                                        <>
                                            <div className="mt-3 flex items-center gap-3">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    <div className="truncate font-display text-lg font-semibold text-white">{summary.topClub.name}</div>
                                                    <div className="truncate text-xs text-[#94A3B8]">{summary.topClub.organization ?? '—'}</div>
                                                </div>
                                            </div>
                                            <div className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-[#FFF3EB] px-2 py-1">
                                                <Award className="h-3 w-3 text-[#FF6B00]" />
                                                <span className="font-mono text-xs font-semibold text-[#CC5500]">{summary.topClub.count}</span>
                    ...
                    <CardContent className="p-0">
                        <div className="overflow-x-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow className="border-[#2A2A2A] hover:bg-transparent">
                                        <TableHead className="px-6 py-3 text-xs font-semibold uppercase tracking-wider text-[#94A3B8]">Club</TableHead>
                                ...
                                <TableBody>
                                    ...
                                    {ratings.data.map((rating) => {
                                        if (rating.scout.role !== 'club') return null;
                                        return (
                ...
                <div className="grid grid-cols-1 gap-6">
                    <Card className="border-[#2A2A2A] bg-[#0F0F0F]">
                        <CardHeader className="border-b border-[#2A2A2A] p-6">
                            ...
                            <p className="mt-1 text-xs text-[#94A3B8]">Players with the highest number of submitted ratings by clubs</p>
                        </CardHeader>
                        <CardContent className="p-0">
                            ...
                            <div className="space-y-5">
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    <div className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] p-4">
                                        <div className="mb-3 text-xs font-medium uppercase tracking-wider text-[#94A3B8]">Club</div>
                                        <div className="flex items-center gap-3">
                                ...
                                {viewRating.notes && (
                                    <div>
                                        <div className="mb-2 text-xs font-medium uppercase tracking-wider text-[#94A3B8]">Club Notes</div>

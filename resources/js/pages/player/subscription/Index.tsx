        ...
        if (plan.id === 'premium') {
            return (
                <button
                    ...
                    className={`w-full rounded-lg px-5 py-3 font-sans text-sm font-semibold transition-colors ${disablePremium
                        ? 'cursor-not-allowed bg-[#F8FAFC] text-[#94A3B8] dark:bg-[#1F1F1F] dark:text-[#555555]'
                        : 'bg-[#FF6B00] text-white hover:bg-[#CC5500]'
                        }`}
                >
                    {disablePremium ? 'Already Subscribed' : plan.cta}
                </button>
            );
        }
        if (plan.id === 'elite') {
            return (
                <button
                    ...
                    className={`w-full rounded-lg px-5 py-3 font-sans text-sm font-semibold transition-colors ${disableElite
                        ? 'cursor-not-allowed border border-[#E2E8F0] bg-[#F8FAFC] text-[#94A3B8] dark:border-[#2A2A2A] dark:bg-[#1F1F1F] dark:text-[#555555]'
                ...
                >
                    {disableElite ? 'Already Subscribed' : plan.cta}
                </button>
            );
        }
        // free (kintu current na — mane user upgrade kore ache)
        return (
            <button
                disabled
                className="w-full cursor-not-allowed rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-5 py-3 font-sans text-sm font-semibold text-[#94A3B8] dark:border-[#2A2A2A] dark:bg-[#1F1F1F] dark:text-[#555555]"
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-5 lg:gap-6">
                            {plans.map((plan) => {
                                ...
                                return (
                                    <div
                                        key={plan.id}
                                        className={`relative flex flex-col rounded-2xl border bg-white p-6 dark:bg-[#161616] sm:p-7 ${plan.highlighted
                                        ...
                                        <ul className="mb-7 flex-1 space-y-3">
                                            {plan.features.map((feature, idx) => (
                                                <li key={idx} className="flex items-start gap-2.5">
                                                    {feature.included ? (
                                                        <div
                                                            className={`mt-0.5 flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full ${feature.highlight
                                                                ? 'bg-[#FF6B00]'
                                                                : 'bg-[#16A34A]/10 dark:bg-[#16A34A]/20'
                                                                }`}
                                                        >
                                                            <Check
                                                                className={`h-2.5 w-2.5 ${feature.highlight ? 'text-white' : 'text-[#16A34A]'
                                                                    }`}
                                                                strokeWidth={3.5}
                                                            />
                                                        </div>
                                                    ...
                                                    <span
                                                        className={`font-sans text-sm leading-tight ${feature.included
                                                            ? feature.highlight
                                        ...
                                        <tr className="border-t-2 border-[#E2E8F0] bg-[#F8FAFC] dark:border-[#2A2A2A] dark:bg-[#1F1F1F]">
                                            ...
                                            <td className="px-4 py-5 text-center">
                                                <span className="font-mono text-[10px] uppercase tracking-wider text-[#94A3B8] dark:text-[#555555]">
                                                    {currentPlanId === 'free' ? 'Current' : 'Free'}
                                                </span>
                                            </td>
                                            <td className="bg-[#FFF3EB] px-4 py-5 text-center dark:bg-[rgba(255,107,0,0.08)]">
                                                <button
                                                    onClick={disablePremium ? undefined : () => handleCheckout('plan_one')}
                                                    disabled={disablePremium}
                                                    className={`w-full rounded-lg px-3 py-2.5 font-sans text-xs font-semibold text-white ${disablePremium
                                                        ? 'cursor-not-allowed bg-gray-500 opacity-60'
                                                        : 'bg-[#FF6B00] hover:bg-[#CC5500]'
                                                        }`}
                                                >
                                                    {disablePremium ? 'Subscribed' : 'Upgrade'}
                                                </button>
                                            </td>
                                            <td className="px-4 py-5 text-center">
                                                <button
                                                    onClick={disableElite ? undefined : () => handleCheckout('plan_two')}
                                                    disabled={disableElite}
                                                    className={`w-full rounded-lg border px-3 py-2.5 font-sans text-xs font-semibold ${disableElite
                                                        ? 'cursor-not-allowed border-gray-300 text-[#94A3B8] opacity-60 dark:border-[#2A2A2A] dark:text-[#555555]'
                                                        : 'border-[#0F172A] bg-white text-[#0F172A] hover:bg-[#0F172A] hover:text-white dark:border-[#F5F5F5] dark:bg-transparent dark:text-[#F5F5F5] dark:hover:bg-[#F5F5F5] dark:hover:text-[#0F172A]'
                                                        }`}
                                                >

...
function PlayerNavbar() {
    var _a, _b, _c, _d;
    var _e = react_1.usePage(), url = _e.url, props = _e.props;
    // TODO: const { auth } = usePage<PageProps>().props;
    // const auth = props.auth ?? {
    //     user: { id: 1, name: 'Lucas Pereira', email: 'lucas@hilights.fc', avatar_url: null },
    // };
    var auth = ((_a = props.auth) === null || _a === void 0 ? void 0 : _a.user) ? props.auth
        : {
            user: {
                id: 1,
                name: 'Lucas Pereira',
    ...
    var notificationCount = (_b = props.notificationCount) !== null && _b !== void 0 ? _b : 3;
    var _f = react_2.useState(false), scrolled = _f[0], setScrolled = _f[1];
    var _g = react_2.useState(false), mobileOpen = _g[0], setMobileOpen = _g[1];
    ...
    var handleLogout = function (e) {
        ...
        react_1.router.post('/logout');
    };
    //TODO change after real authentication
    // const initials = getInitials(auth.user.name);
    var initials = getInitials((_d = (_c = auth === null || auth === void 0 ? void 0 : auth.user) === null || _c === void 0 ? void 0 : _c.name) !== null && _d !== void 0 ? _d : 'Player');
    return (react_2["default"].createElement("header", { className: [
        ...
        react_2["default"].createElement("div", { className: "mx-auto flex h-full max-w-[1400px] items-center justify-between gap-4 px-2 sm:px-6" },
            react_2["default"].createElement(react_1.Link, { href: "/", className: "flex shrink-0 items-center gap-2", "aria-label": "HiLights Football dashboard" },
                react_2["default"].createElement("img", { src: "/images/logo/final_logo.png", className: "hidden h-6 w-auto sm:h-8 lg:h-10 xl:h-12 2xl:h-14 dark:block", alt: "HiLights Football" })),
            react_2["default"].createElement("nav", { className: "hidden items-center gap-8 md:flex" }, NAV_LINKS.map(function (link) {
                    ...
                    react_2["default"].createElement(dropdown_menu_1.DropdownMenuContent, { align: "end", className: "w-56 rounded-xl border border-[#E2E8F0] bg-white p-1 shadow-lg dark:border-[#2A2A2A] dark:bg-[#161616]" },
                        ...
                        react_2["default"].createElement(dropdown_menu_1.DropdownMenuItem, { asChild: true, className: "cursor-pointer rounded-md focus:bg-[#F8FAFC] dark:focus:bg-[#1F1F1F]" },
                            react_2["default"].createElement(react_1.Link, { href: "/players/" + auth.user.id, className: "flex items-center gap-2 px-3 py-2 text-sm text-[#0F172A] dark:text-[#F5F5F5]" },
                                react_2["default"].createElement(lucide_react_1.User, { className: "h-4 w-4 text-[#94A3B8]" }),
                                "View Public Profile")),
                        react_2["default"].createElement(dropdown_menu_1.DropdownMenuItem, { asChild: true, className: "cursor-pointer rounded-md focus:bg-[#F8FAFC] dark:focus:bg-[#1F1F1F]" },
                            react_2["default"].createElement(react_1.Link, { href: "/player/settings", className: "flex items-center gap-2 px-3 py-2 text-sm text-[#0F172A] dark:text-[#F5F5F5]" },
                                react_2["default"].createElement(lucide_react_1.Settings, { className: "h-4 w-4 text-[#94A3B8]" }),
                                "Account Settings")),
                        react_2["default"].createElement(dropdown_menu_1.DropdownMenuSeparator, { className: "my-1 bg-[#F1F5F9] dark:bg-[#1F1F1F]" }),
                        react_2["default"].createElement(dropdown_menu_1.DropdownMenuItem, { onClick: handleLogout, className: "flex cursor-pointer items-center gap-2 rounded-md px-3 py-2 text-sm text-red-500 focus:bg-red-50 focus:text-red-600 dark:text-red-400 dark:focus:bg-[rgba(220,38,38,0.1)] dark:focus:text-red-400" },
                            react_2["default"].createElement(lucide_react_1.LogOut, { className: "h-4 w-4" }),
                            "Logout")))),
            react_2["default"].createElement("div", { className: "flex items-center gap-1 md:hidden" },
                react_2["default"].createElement(react_1.Link, { href: "/player/notifications", className: "relative rounded-md p-2 transition-colors hover:bg-[#F8FAFC] dark:hover:bg-[#1F1F1F]", "aria-label": "Notifications" },
                    react_2["default"].createElement(lucide_react_1.Bell, { className: "h-5 w-5 text-[#94A3B8] dark:text-[#555555]" }),
                    notificationCount > 0 && (react_2["default"].createElement("span", { className: "absolute top-1 right-1 h-2 w-2 rounded-full bg-red-500 ring-2 ring-white dark:ring-[#0D0D0D]" }))),
                react_2["default"].createElement(ThemeToggle_1["default"], null),
                react_2["default"].createElement(sheet_1.Sheet, { open: mobileOpen, onOpenChange: setMobileOpen },
                    react_2["default"].createElement(sheet_1.SheetTrigger, { asChild: true },
                        react_2["default"].createElement(button_1.Button, { variant: "ghost", size: "icon", "aria-label": "Open menu", className: "h-9 w-9 text-[#0F172A] hover:bg-[#F8FAFC] dark:text-[#F5F5F5] dark:hover:bg-[#1F1F1F]" },
                    ...
                    react_2["default"].createElement(sheet_1.SheetContent, { side: "left", className: "flex w-[300px] flex-col border-r border-[#E2E8F0] bg-white p-0 dark:border-[#2A2A2A] dark:bg-[#0D0D0D]" },
                        react_2["default"].createElement(sheet_1.SheetHeader, { className: "border-b border-[#E2E8F0] px-6 py-4 dark:border-[#2A2A2A]" },
                            react_2["default"].createElement(sheet_1.SheetTitle, { className: "flex items-center gap-2" },
                                react_2["default"].createElement("img", { src: "/images/logo/hilights_logo_transparent_200.png", className: "h-9 w-auto dark:hidden", alt: "HiLights Football" }),
                        ...
                        react_2["default"].createElement("div", { className: "mt-4 border-t border-[#E2E8F0] px-6 py-4 dark:border-[#2A2A2A]" },
                            react_2["default"].createElement(button_1.Button, { onClick: function (e) {
                                    handleLogout(e);
                                    setMobileOpen(false);
                                }, variant: "outline", className: "w-full gap-2 border-red-200 bg-transparent text-sm font-medium text-red-500 hover:border-red-300 hover:bg-red-50 hover:text-red-600 dark:border-red-900/40 dark:text-red-400 dark:hover:bg-[rgba(220,38,38,0.1)]" },
                                react_2["default"].createElement(lucide_react_1.LogOut, { className: "h-4 w-4" }),
                                "Logout"))))))));
}
exports["default"] = PlayerNavbar;

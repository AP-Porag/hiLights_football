...
class RatingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = PlayerRating::query()->with([
            // ✅ Only select columns that actually exist in `users`
            'scout:id,name',   // no avatar_url, organization, country
            'playerProfile.user',
        ]);
        ...
        $mostRatedPlayers = $mostRated->map(function ($row) use ($profiles) {
            ...
            return [
                ...
                'avatar'   => $profile?->photo_url,   // this is from PlayerProfile
            ];
        })->values();
        ...
        $scoutList = User::whereIn('id', PlayerRating::distinct()->pluck('scout_id'))
            ->orderBy('name')
            ->get(['id', 'name'])
